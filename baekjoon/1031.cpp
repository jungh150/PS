// Reference: green55 teamnote
// https://github.com/green5555/Teamnote/blob/master/TeamNote/Ed-Karp.cpp

#include <bits/stdc++.h>
using namespace std;

typedef pair<int, int> pii;

const int MAX = 110, INF = INT_MAX;
struct maxflow {
    struct edge {
        int next, cap, flow = 0, rev_idx;
        edge() {}
        edge(int n, int c) : next(n), cap(c) {}
        int remain() {
            return cap - flow;
        }
    };
    vector<edge> adj[MAX];
    void makeEdge(int u, int v, int c) {
        adj[u].emplace_back(v, c);
        adj[v].emplace_back(u, 0);
        adj[u].back().rev_idx = adj[v].size() - 1;
        adj[v].back().rev_idx = adj[u].size() - 1;
    }
    int parent[MAX];
    pii path[MAX];
    int solve(int S, int E) {
        int ans = 0;
        queue<int> q;
        while (1) {
            memset(parent, -1, sizeof(parent));
            q.push(S);
            while (!q.empty()) {
                int u = q.front(); q.pop();
                for (int i = 0; i < adj[u].size(); ++i) {
                    auto &e = adj[u][i];
                    if (e.remain() > 0 && parent[e.next] == -1) {
                        parent[e.next] = u;
                        path[e.next] = { u, i };
                        q.emplace(e.next);
                    }
                }
            }
            if (parent[E] == -1) break;
            int ret = INF;
            for (int i = E; i != S; i = parent[i])
                ret = min(ret, adj[path[i].first][path[i].second].remain());
            for (int i = E; i != S; i = parent[i]) {
                auto &e = adj[path[i].first][path[i].second];
                e.flow += ret;
                adj[e.next][e.rev_idx].flow -= ret;
            }
            ans += ret;
        }
        return ans;
    }
    bool changeFlow(int S, int E) {
        int idx = -1;
        for (int i = 0; i < adj[S].size(); ++i) {
            if (adj[S][i].next == E && adj[S][i].cap > 0) {
                idx = i;
                break;
            }
        }
        memset(parent, -1, sizeof(parent));
        queue<int> q;
        parent[S] = S;
        q.push(S);
        while (!q.empty()) {
            int u = q.front(); q.pop();
            for (int i = 0; i < adj[u].size(); ++i) {
                auto &e = adj[u][i];
                if (u < S || (u == S && e.next < E)) continue;
                if (e.remain() > 0 && parent[e.next] == -1) {
                    parent[e.next] = u;
                    path[e.next] = { u, i };
                    q.emplace(e.next);
                }
            }
        }
        if (parent[E] == -1) return false;
        adj[S][idx].flow -= 1;
        adj[E][adj[S][idx].rev_idx].flow += 1;
        for (int i = E; i != S; i = parent[i]) {
            auto &e = adj[path[i].first][path[i].second];
            e.flow += 1;
            adj[e.next][e.rev_idx].flow -= 1;
        }
        return true;
    }
};

void solve() {
    maxflow mf;

    int n, m;
    cin >> n >> m;

    int need = 0;

    for (int i = 1; i < n + 1; i++) {
        int x;
        cin >> x;
        mf.makeEdge(108, i, x);
    }

    for (int i = 1; i < m + 1; i++) {
        int x;
        cin >> x;
        need += x;
        mf.makeEdge(i + 50, 109, x);
    }

    for (int i = 1; i < n + 1; i++) {
        for (int j = 1; j < m + 1; j++) {
            mf.makeEdge(i, j + 50, 1);
        }
    }

    if (mf.solve(108, 109) != need) {
        cout << -1 << '\n';
        return;
    }

    for (int i = 1; i < n + 1; i++) {
        for (int j = 1; j < m + 1; j++) {
            mf.changeFlow(i, j + 50);
        }
    }

    for (int i = 1; i < n + 1; i++) {
        for (int j = 1; j < m + 1; j++) {
            int x = 0;
            for (auto &e: mf.adj[i]) {
                if (e.next == j + 50 && e.cap > 0) {
                    x = e.flow;
                    break;
                }
            }
            cout << x;
        }
        cout << '\n';
    }
}

int main() {
    ios_base::sync_with_stdio(0);
    cin.tie(0);

    int T = 1;
    for (int i = 0; i < T; i++) {
        solve();
    }
}