...
void solve() {
    ...
    for (int i = 1; i <= 400; i++) {
        mf.makeEdge(i * 2 - 1, i * 2, 1); // i의 in -> i의 out
        mf.makeEdge(i * 2, i * 2 - 1, 1); // i의 out -> i의 in
    }

    while (p--) {
        int a, b;
        cin >> a >> b;
        mf.makeEdge(a * 2, b * 2 - 1, 1); // a의 out -> b의 in
        mf.makeEdge(b * 2, a * 2 - 1, 1); // b의 out -> a의 in
    }

    cout << mf.solve(1 * 2, 2 * 2 - 1) << '\n';
}
