...
struct SegTree {
    ...
    long long update(int idx, int l, int r, int target, long long val) {
        ...
        int m = (l + r) / 2;
        long long res = update(idx * 2, l, m, target, val) + update(idx * 2 + 1, m + 1, r, target, val);
        if (res >= MOD) res -= MOD;
        return tree[idx] = res;
    }
    ...
    long long query(int idx, int l, int r, int wl, int wr) {
        ...
        int m = (l + r) / 2;
        long long res = query(idx * 2, l, m, wl, wr) + query(idx * 2 + 1, m + 1, r, wl, wr);
        if (res >= MOD) res -= MOD;
        return res;
    }
