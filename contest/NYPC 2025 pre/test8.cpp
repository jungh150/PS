...
public:
    ...
    Bid calculateBid(const vector<int>& diceA, const vector<int>& diceB) {
        ...
        // ===== Hard-deny trigger (PATCH) =====
        // 상대가 이번 턴에 SS/LS/YACHT를 바로 성립시킬 수 있으면 → 무조건 경합 전환 + 최소입찰 하한
        bool hardDeny = false;
        if (oppImmLower && (oppImmRule==SMALL_STRAIGHT || oppImmRule==LARGE_STRAIGHT || oppImmRule==YACHT)) {
            hardDeny = true;
            myPref = oppPref;        // 내 선호를 상대 선호로 강제 전환
            contested = true;        // 경합 확정
        }

        // ===== Amount calc =====
        if (!contested || cap <= 0) {
            // 비경합 TAX (PATCH): 델타 기반 과세 + 바닥세
            // beta_tax * Δ를 cap 이내로 과세
            double beta_tax = 0.25;            // 요청값
            int taxCapEarly = 1000;            // 요청값
            int taxCapLate  = 2000;            // 요청값
            ...
            // 바닥세: amount >= min(300, taxCap) (단, cap 이내)
            int floorTax = min(300, taxCap);
            amount = max(amount, floorTax);
            amount = min(amount, cap); // 항상 cap 이내
        } else {
            // 경합
            if (oppLikelyZero) {
                // Tie-break ε
                ...
                // Proportional bid with cap (기존 로직 유지)
            ...
            // 경합 바닥세 (PATCH): cap>0 이면 최소 180 보장(단, cap 이내)
            if (cap > 0) amount = max(amount, min(180, cap));

            // Hard-deny가 걸린 경우 추가 하한 (PATCH):
            if (hardDeny && cap > 0) {
                int minBid = min(cap, (int)llround(900 + 1200 * progress)); // 초반~후반 900~2100
