...
int main() {
	...
	while (getline(cin, line)) {
		...
		if (cmd == "READY") {
			...
			i64 d_us = llabs(UA - UB);
			i64 d_op = llabs(OA - OB);
			int round_no = __builtin_popcount(mask_us) + 1;
			if (round_no <= 2) d_us = (i64)llround(1.6 * d_us);
			i64 lead = (score_us + bidbal_us) - (score_op + bidbal_op);
			i64 cap = (i64)(0.95 * (double)min(d_us, d_op));
			...
			if (pick_us != pick_op) {
				...
				double base = p_contest * (double)d_us * 0.5;
				...
				bool refund_mode = (lead < -30000) && ema_init &&
								   (d_us < 15000) && (p_contest > 0.8) &&
								   (ema_opp > 1500.0);
				if (refund_mode) raw = min((double)cap, max(1.0, ema_opp - 1.0));
				bid = (i64)llround(raw);
