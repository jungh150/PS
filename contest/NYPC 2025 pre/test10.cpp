...
public:
    ...
    Bid calculateBid(const vector<int>& diceA, const vector<int>& diceB) {
        ...
        int scoreDiff = oppTotal - myTotal;
        double progress = min(1.0, max(0.0, (rollCount - 1) / 11.0));
        
        int cap = max(0, delta / 2);
        int amount = 0;
        
        auto tau_deny = (int)llround(600 + 1000 * progress);      // 완화
        auto tau_delta = (int)llround(2000 + 3000 * progress);     // 완화
        ...
        if (oppImmLower && (oppImmRule==SMALL_STRAIGHT || oppImmRule==LARGE_STRAIGHT || oppImmRule==YACHT)) {
            if (cap >= tau_deny && delta >= tau_delta) {
                ...
                if (valOppPref >= (int)(0.3 * myBest)) { // 완화
                    hardDeny = true;
