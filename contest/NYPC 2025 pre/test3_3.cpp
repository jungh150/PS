// 제출번호 83370: 4승 6패
// test3.cpp 기반
// "ONE/TWO 보호"에서 THREE까지 보호하도록 확장
// Upper-first Gate 적용
...
static EvalResult evaluateBestPlay(const GameState& st, const vector<int>& inventory) {
    EvalResult upperBest, lowerBest;
    bool hasU = false, hasL = false;
    int n = (int)inventory.size();
    if (n < 5) return EvalResult{};
    ...
    for (int r = 0; r < 12; ++r) {
        ...
        for (auto id : combs) {
            ...
            // 상단 / 하단 각각 최고 후보 갱신
            if (r >= ONE && r <= SIX) {
                if (!hasU || sc + extra + shape > upperBest.effective()) {
                    upperBest.rule = (DiceRule)r;
                    upperBest.dice = hand;
                    upperBest.score = sc;
                    upperBest.extraBonus = extra;
                    upperBest.shaping = shape;
                    hasU = true;
                }
            } else {
                if (!hasL || sc + extra + shape > lowerBest.effective()) {
                    lowerBest.rule = (DiceRule)r;
                    lowerBest.dice = hand;
                    lowerBest.score = sc;
                    lowerBest.extraBonus = extra;
                    lowerBest.shaping = shape;
                    hasL = true;
                }
            }
        }
    ...
    // ---- Upper-first Gate (최종 선택) ----
    EvalResult best;
    if (needUpper > 0) {
        int M = (int)llround(0.15 * needUpper + 1200.0 * (double)unusedUpper);
        // 하단이 상단보다 M 이상 좋아야만 하단 채택
        if (hasU && (!hasL || lowerBest.effective() < upperBest.effective() + M)) {
            best = upperBest;
        } else if (hasL) {
            best = lowerBest;
        }
    } else {
        if (hasU && (!hasL || upperBest.effective() >= lowerBest.effective())) {
            best = upperBest;
        } else if (hasL) {
            best = lowerBest;
        }
