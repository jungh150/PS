// 제출번호 81855: 2승 8패
// Yacht Auction AI (C++20, bonus-aware v3: tuned vs bots #6/#7)
// - Compatible I/O with sample-code.cpp
// - Key changes targeting #6/#7 based on failure analysis:
//   * Stronger "upper-section bonus path" shaping (encourage reaching 63,000)
//   * Stronger penalty for choosing non-upper when upper still needed
//   * Late-game deadline rule: punish non-upper when few upper slots remain
//   * ONE/TWO late protection boosted (avoid 0-point upper at the end)
//   * Bidding: margin-based contested detection + opponent aggressiveness tracking
//       - amount ≤ floor(0.5 * Δ)
//       - consider opponent’s typical contested bids (quantile) to avoid being outbid
//       - add extra weight if this bundle helps upper-path a lot
//
...
// === Bonus-aware evaluator (tuned) ===
// Heuristics tuned to beat aggressive upper-focused bots (#6/#7):
//  - Path bonus for upper:   K_path * min(sc, needUpper)        [↑ stronger]
//  - Non-upper penalty:      -K_nonup * urgency * needUpper/unusedUpper  [↑ stronger]
//  - Deadline penalty: if (unusedUpper<=2 && needUpper>0 && non-upper) → extra big penalty
//  - ONE/TWO late protection: K_low * urgency_low when sc>0 and needUpper>0 [↑ stronger]
static EvalResult evaluateBestPlay(const GameState& st, const vector<int>& inventory) {
    ...
    // Tuned weights
    const double K_path   = 0.55;                 // ↑ stronger than v2
    const double K_nonup  = 0.40;                 // ↑ stronger than v2
    const double urgency  = (unusedUpper <= 2 ? 1.15 : (0.5 + 0.6*progress));
    const double K_low    = 5000.0;               // ↑ late protection for ONE/TWO
    const int DEADLINE_PENALTY = 12000;           // big penalty when few upper slots left
        ...
        for (auto id : combs) {
            ...
            if (r>=ONE && r<=SIX) {
                ...
                if (needUpper > 0 && unusedUpper > 0) {
                    ...
                    shape -= pen;
                    // Deadline penalty when very few upper slots remain
                    if (unusedUpper <= 2) shape -= DEADLINE_PENALTY;
                }
            }
...
public:
    ...
    vector<int> oppContestedBids;         // opponent amounts in truly contested rounds (observed)
    Bid lastMyBid{'A', 0};

    // ---- Bidding (tuned vs #6/#7) ----
    Bid calculateBid(const vector<int>& diceA, const vector<int>& diceB) {
        ...
        // Margin-based contested prediction
        const int CONTEST_MARGIN = 4000; // if opponent values are close, treat as contested
        ...
        // Absolute safety cap to avoid rare huge spends late
        ...
        if (!contested || cap <= 0) {
            ...
            // If opponent seems zero-bidder, prefer tiny ε
            ...
            // Ensure non-negative & within hard bounds
