...
struct board {
    board() : used_mask(0), cnt{}, score_up(0), score_down(0) {}
    ...
    // 메인 선택(② CHOICE/STRAIGHT 억제, ③ 상단 품질+0점 방지, Upper-first Gate)
    auto choose() {
        // 빠른 YACHT
        ...
        int round = __builtin_popcount((unsigned)used_mask); // 0..11 (이번 선택 전 상태)
        int unusedUpper = 0; for (int i=0;i<=5;i++) if (((used_mask>>i)&1)==0) unusedUpper++;
        int needUpper = max(0, 63000 - score_up); // calc에서 넘으면 +35000 이미 포함됨
        ...
        auto rec = [&](const auto& self, int dep, string acc)->void{
            if (dep==7) {
                ...
                for (int op=0; op<=11; op++) {
                    ...
                    int sc = calc(op, acc); // 기본 점수(+보너스 crossing 시 35k 포함)
                    int val = sc;
                    bool isUpper = (op<=5);

                    // --- ③ 0점 방지: ONE/TWO/THREE에 해당 눈이 없으면 사실상 금지 ---
                    if (op==0 && c1==0) val = -1000000000;
                    if (op==1 && c2==0) val = -1000000000;
                    if (op==2 && c3==0) val = -1000000000;

                    // 상단 얕은 채우기 약화(FOUR/FIVE/SIX 보호 강화)
                    if (op==3 && c4<=1 && needUpper>0) val -= 4000;           // FOUR가 1개뿐이면 감점
                    if (op==4 && c5<=2 && five_unused)  val -= 10000;         // FIVE 재료 부족시 감점 강화
                    if (op==5 && c6<=2 && six_unused)   val -= 12000;         // SIX 재료 부족시 더 큰 감점

                    // --- ② 초반 CHOICE 강력 억제 & 5/6 소모 억제 ---
                    if (op==6 /*CHOICE*/ && round<=4) val -= 20000;          // 라운드 1~4 강한 페널티
                    if (op==6 /*CHOICE*/ && (five_unused||six_unused)) {
                        // CHOICE가 5/6을 소모하면 더 감점
                        val -= 2000*c5 + 3000*c6;
                    }

                    // --- ② 스트레이트 보정: 5/6 많이 쓰면 감점, 적게 쓰면 소폭 가점 ---
                    if (op==9 || op==10) {
                        bool early = (round<=3);
                        if (use56==0)       val += early ? 5000 : 3000;
                        else if (use56==1)  val += early ? 2000 : 1000;
                        else { // 5/6을 2개 이상 소비
                            if (five_unused||six_unused) val -= 2500*min(3,use56);
                        }
                    }

                    // --- 하단에서 5/6 과소비 감점 (보너스 경로 보호) ---
                    if (op>5 && (five_unused||six_unused)) {
                        val -= 2000*c5 + 3000*c6;
                    }

                    // --- 비상단 선택 시, 상단 미충족이면 긴급도 페널티 ---
                    if (op>5 && needUpper>0 && unusedUpper>0) {
                        int pen = (int)llround(0.25 * (double)needUpper / max(1, unusedUpper));
                        val -= pen;
                    }

                    // --- ONE/TWO/THREE 소형 보호(막판 0방지): 점수>0이고 상단 부족 시 미세 가점 ---
                    if ((op==0||op==1||op==2) && sc>0 && needUpper>0 && unusedUpper<=2) {
                        val += 3000;
                    }

                    // --- 소형 MC로 다음 한 수 기대치 보정(과하지 않게) ---
                    {
        ...
        // ---- Upper-first Gate ----
        // 상단 미충족이면: 하단이 상단보다 충분히 커야만 하단 채택
        if (needUpper > 0) {
            int M = (int)llround(0.15 * needUpper + 1200.0 * (double)unusedUpper);
...
int main() {
    ...
    while (true) {
        ...
        if (cmd=="READY") {
            ...
            if (val_final > 10000) val_final = 10000; // 과투입 방지
