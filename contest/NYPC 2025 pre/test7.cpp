...
struct Tuning {
    ...
    double alpha_cap     = 0.75;  // α in cap = 0.5 * (Δ_me + α * Δ_opp)
    double beta_oppon    = 0.35;  // β for mixing Δ_opp into base amount: round((Δ_me + β*Δ_opp) * f)
    // Urgency vs score diff factor upper bound
    double f_scorediff_cap = 0.45; // was 0.15 in v2; increase to fight aggro
    // Non-contested "tax" when we don't contest
    double beta_tax      = 0.18;   // tax ~ β_tax * Δ_opp (clamped)
    int    taxCapEarly   = 600;    // cap tax by stage
    int    taxCapLate    = 1400;

    // Defensive contest switch (deny)
    double theta_deny    = 0.9;    // thresholds dopp*U > theta * selfLoss
