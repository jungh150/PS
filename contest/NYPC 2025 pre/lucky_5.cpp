...
string get_op_name(int op) {
    ...
    return (op >= 0 && op < 12) ? names[op] : "UNKNOWN";
}
...
struct board {
    ...
    auto choose() {
        ...
        auto rec = [&](const auto& self, int dep, string acc)->void{
            if (dep==7) {
                ...
                for (int op=0; op<=11; op++) {
                    ...
                    if ((op==0||op==1||op==2) && sc>0 && needUpper>0 && unusedUpper<=3) val += 3500;
                    if (op==3 && c4<=1 && needUpper>0) val -= 5000;

                    if (op==4 && c5<=2 && five_unused)  val -= 15000;
                    if (op==5 && c6<=2 && six_unused)   val -= 20000;

                    if (op==6 && round<=4) val -= 25000;
                    ...
                    if (op==9 || op==10) {
                        bool early = (round<=3);
                        if (use56==0) val += early ? 4000 : 2000;
                        else {
        ...
        if (needUpper > 0) {
            int M = (int)llround(0.25 * needUpper + 1800.0 * (double)unusedUpper + 5000.0);
...
private:
    ...
    int calc(int op, const string& s) const {
        ...
        if (op==9)  return is_small_straight_str(s) ? 15000 : 0;
        if (op==10) return is_large_straight_str(s) ? 30000 : 0;
...
int main() {
    ...
    while (true) {
        ...
        if (cmd=="READY") { cout << "OK" << endl; }
            ...
            int val_final = min(val, 5000);
            if (val_final < 300) val_final = 300;
            f.roll(a, b); g.roll(a, b);
            if (prefer == 'A') { cout << "BID A " << val_final << endl; f_prv_type=0; f_prv_bid=val_final; }
            else { cout << "BID B " << val_final << endl; f_prv_type=1; f_prv_bid=val_final; }
        }
        else if (cmd=="GET") {
