...
struct board {
    ...
    auto choose() {
        ...
        auto rec = [&](const auto& self, int dep, string acc)->void{
            if (dep==7) {
                ...
                for (int op=0; op<=11; op++) {
                    ...
                    // 막판(상단 남은 슬롯 적음) ONE/TWO/THREE 양수면 소폭 가점
                    if ((op==0||op==1||op==2) && sc>0 && needUpper>0 && unusedUpper<=3) val += 4000;
                    ...
                    if (op==5 && c6<=2 && six_unused)   val -= 18000;

                    // --- CHOICE 초반 강한 억제 + 5/6 소모 벌점 ---
                    if (op==6 && round<=4) val -= 30000;
                    if (op==6 && (five_unused || six_unused)) {
                        val -= 2500*c5 + 3500*c6; // 6을 더 강하게 보호
                    }

                    // --- 스트레이트 보정 ---
                    if (op==9 || op==10) {
                        bool early = (round<=3);
                        if (use56==0)       val += early ? 4000 : 2500;  // 5/6 안 쓰면만 소폭 가점
                        else { // 5/6을 쓰면 감점(특히 6)
        ...
        // ---- Upper-first Gate 더 강하게 ----
        if (needUpper > 0) {
            int M = (int)llround(0.22 * needUpper + 1800.0 * (double)unusedUpper + 4000.0);
