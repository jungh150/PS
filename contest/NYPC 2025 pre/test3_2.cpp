    ...
    for (int r=0; r<12; ++r) {
        ...
        for (auto id : combs) {
            ...
            if (r>=ONE && r<=SIX) {
                ...
                if ((r==ONE || r==TWO || r==THREE) && sc>0 && needUpper>0) {
                    const double k_low_one_two = 2500.0;
                    const double k_low_three   = 1800.0; // THREE는 조금 약하게, 필요시 2200~2500로 상향
                    double urg_low = (unusedUpper<=2 ? 1.0 : progress);
                    double bonus = (r==THREE ? k_low_three : k_low_one_two) * urg_low;
                    shape += (int)llround(bonus);
                }
            } else {
