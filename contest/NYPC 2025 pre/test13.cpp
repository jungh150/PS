...
// === Our evaluator (test3의 bonus-aware v2 그대로) ===
...
// ===== Immediate-lower detector for opponent =====
// 상대가 이번 턴에 SS/LS/YACHT를 낼 수 있으면 true (해당 규칙이 미사용이어야 함)
static bool canImmediateLower(const GameState& st, const vector<int>& inventory) {
    int n = (int)inventory.size();
    if (n < 5) return false;

    bool needSS = (st.ruleScore[SMALL_STRAIGHT] == -1);
    bool needLS = (st.ruleScore[LARGE_STRAIGHT] == -1);
    bool needY  = (st.ruleScore[YACHT] == -1);
    if (!needSS && !needLS && !needY) return false;
    ...
    for (auto id : combs) {
        vector<int> hand = { inventory[id[0]],inventory[id[1]],inventory[id[2]],
                             inventory[id[3]],inventory[id[4]] };
        if (needSS && GameState::calculateScoreStatic(SMALL_STRAIGHT, hand) > 0) return true;
        if (needLS && GameState::calculateScoreStatic(LARGE_STRAIGHT, hand) > 0) return true;
        if (needY  && GameState::calculateScoreStatic(YACHT, hand) > 0)          return true;
    }
    return false;
}
...
public:
    ...
    Bid calculateBid(const vector<int>& diceA, const vector<int>& diceB) {
        ...
        // Opponent preference (rough, same evaluator as ours)
        ...
        // ===== Minimal Hard-deny: 상대가 선호하는 번들로 SS/LS/YACHT 즉시 가능? =====
        // 가능하면 그 번들로 경합만 강제(입찰액 하한 없음).
        {
            vector<int> merged = oppSeed;
            if (oppPref == 'A') merged.insert(merged.end(), diceA.begin(), diceA.end());
            else                merged.insert(merged.end(), diceB.begin(), diceB.end());

            if (canImmediateLower(oppState, merged)) {
                myPref = oppPref;
                contested = true;
                // (입찰액 계산은 기존 규칙 그대로, 하한/세금 없음)
            }
        }
